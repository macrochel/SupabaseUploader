...
def start(response):
    ...
    for i in range(0, len(response), 1):
        ...
        if not response_item.get("implementation_item_category1"):
            response_item["implementation_item_category1"] = "Не указано"
        if not response_item.get("implementation_item_category2"):
            response_item["implementation_item_category2"] = "Не указано"
        if not response_item.get("implementation_item_category3"):
            response_item["implementation_item_category3"] = "Не указано"
        if not response_item.get("implementation_item_category4"):
            response_item["implementation_item_category4"] = "Не указано"
        if not response_item.get("implementation_item_category5"):
            response_item["implementation_item_category5"] = "Не указано"
        if response_item.get("implementation_item_category6"):
            response_item["implementation_item_category6"] = response_item["implementation_item_category5"]
        ...
        if response_item.get("implementation_order_time"):
            response_item["implementation_order_time"] = response_item["implementation_order_time"].split(':')[0]
        else:
            product.json.implementation_order_time = "Не указано"

        if response_item.get("implementation_order"):
            response_item["implementation_order"] = response_item["implementation_order"].split(' ')[4]
        if not response_item.get("implementation_order"):
            response_item["implementation_order"] = "_" + generate_random_string(9)

        if not response_item.get("implementation_item"):
            response_item["implementation_item"] = "Без имени"
    ...
    renamed_products = []
    key_mapping = {
        "implementation_order": "order_id",
        "implementation_item": "product_name",
        "warehouse_id": "warehouce",
        "implementation_item_category2": "category1",
        "implementation_item_category3": "category2",
        "implementation_item_category4": "category3",
        "implementation_item_category5": "category4",
        "implementation_order_date": "date",
        "implementation_order_time": "order_hour",
    ...
    for i in range(0, len(return_products), 1):
        ...
        del product["implementation_date"]
        del product["implementation_item_category1"]
    ...
    supabase.table("1S_implementations_all").insert(products).execute()
